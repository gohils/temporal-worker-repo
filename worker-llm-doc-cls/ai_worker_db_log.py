...
def store_document_artifact(
    workflow_id: str, document_url: str, header_id=None, item_id=None, doc_type=None, 
    ocr_raw=None, ocr_result=None, extracted_fields=None, status="NEW", version=1
) -> int:

    query = """
    INSERT INTO workflow_ocr_data (
        workflow_id, header_id, item_id, doc_type, document_url,
        ocr_raw, ocr_result, extracted_fields, status, version, updated_at
    )
    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())
    ON CONFLICT (workflow_id, item_id)
    DO UPDATE SET
        doc_type = COALESCE(EXCLUDED.doc_type, workflow_ocr_data.doc_type),
        document_url = COALESCE(EXCLUDED.document_url, workflow_ocr_data.document_url),
        ocr_raw = COALESCE(EXCLUDED.ocr_raw, workflow_ocr_data.ocr_raw),
        ocr_result = COALESCE(EXCLUDED.ocr_result, workflow_ocr_data.ocr_result),
        extracted_fields = COALESCE(EXCLUDED.extracted_fields, workflow_ocr_data.extracted_fields),
        status = EXCLUDED.status,
        version = workflow_ocr_data.version + 1,
        updated_at = NOW()
    RETURNING ocr_document_id
    """
...
def get_items_details(header_id: int,item_id: int) -> Optional[Dict[str, Any]]:
    """Retrieve a single item linked to a process header."""
    query = "SELECT * FROM automation_process_item WHERE header_id= %s AND id = %s " 

    try:
        with pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(query, (header_id, item_id))
                row = cur.fetchone()
                if row:
