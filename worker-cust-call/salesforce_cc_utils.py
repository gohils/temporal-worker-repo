...
def upsert_opportunity_by_external_id(
    ...
    payload = clean_payload({
        ...
        "AI_Confidence_Score__c": ai_confidence_score * 100,
    ...
    response = requests.patch(upsert_url, json=payload, headers=headers)
...
# CAMPAIGN Enrollment
# =========================
def get_campaign_by_name(token, instance_url, campaign_name):
    url = f"{instance_url}/services/data/{API_VERSION}/query"
    ...
    if not records:
        raise Exception(f"Campaign not found: {campaign_name}")
...
def enroll_contact_into_campaign_by_external_id(
    customer_external_id,
    campaign_name
):
    token, instance_url = get_access_token()
    ...
    # 2. Get Contact (reuse existing logic)
    contact = get_primary_contact(token, instance_url, account_id)

    if not contact:
        raise Exception(f"No contact found for account {customer_external_id}")

    contact_id = contact["Id"]

    # 3. Get Campaign
    campaign = get_campaign_by_name(token, instance_url, campaign_name)
